...
function App() {
	...
	const sellHandler = async (event) => {
		...
		try {
		  ...
		  await landContract.methods.transferFrom(account, NewOwnerAddress, landId).send({ from: account });
	  
		  // Update the building data to reflect the new ownership
		  const buildings = await landContract.methods.getBuildings().call();
		  setBuildings(buildings);
		  
		  setLandOwner(buildings[landId - 1].NewOwnerAddress);
		  setHasOwner(true);
		} catch (error) {
		  window.alert('Error occurred when selling the land');
		}
	}
